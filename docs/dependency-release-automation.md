...
CI caches FetchContent sources and ccache objects. Cache identities include the
exact RocksDB and Nimble commits, compiler identity, container image,
architecture, and dependency build configuration. The raw FetchContent build
tree is intentionally not cached because a local RelWithDebInfo build is much
larger than the reusable source and compiler-object caches.
